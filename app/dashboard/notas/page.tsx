...
export default function NotasPage() {
  ...
  const [formData, setFormData] = useState({ clienteNombre: '', clienteTelefono: '', evento: '', trabajos: [] as Trabajo[], abonos: [] as Abono[], notas: '', asignadaA: '', asignadaNombre: '' });
  ...
  const cargarNotas = async () => {
    ...
    catch { showToast('Error al cargar las notas', 'error'); }
    finally { setLoading(false); }
  };

  const agregarTrabajo = () => {
    const t: Trabajo = { id: `trabajo_${Date.now()}`, producto: '', cantidad: 1, precioUnitario: 0, subtotal: 0, fechaEntrega: { dia: '', mes: '', anio: new Date().getFullYear().toString() }, entregado: false };
  ...
  const eliminarTrabajo = (id: string) => setFormData({ ...formData, trabajos: formData.trabajos.filter(t => t.id !== id) });
  ...
  const limpiarFormulario = () => { setFormData({ clienteNombre: '', clienteTelefono: '', evento: '', trabajos: [], abonos: [], notas: '', asignadaA: '', asignadaNombre: '' }); setEditando(null); };

  const handleEditar = (nota: Nota) => {
    setFormData({ clienteNombre: nota.clienteNombre, clienteTelefono: nota.clienteTelefono, evento: nota.evento || '', trabajos: nota.trabajos, abonos: nota.abonos, notas: nota.notas || '', asignadaA: nota.asignadaA || '', asignadaNombre: nota.asignadaNombre || '' });
    setEditando(nota);
    setVista('formulario');
  };
  ...
  const handleEliminar = (nota: Nota) => {
    ...
    setModalConfirm({
      ...
      onConfirm: async () => {
        ...
        try { await deleteNota(nota.id!, { email: usuarioData.email, nombre: usuarioData.nombre }); await cargarNotas(); showToast(`Nota ${nota.folio} eliminada`, 'info'); }
  ...
  const handleArchivar = (nota: Nota) => {
    ...
    setModalConfirm({
      ...
      onConfirm: async () => {
        if (!usuarioData) return;
        setModalConfirm(null);
        try { await archivarNota(nota.id!, { email: usuarioData.email, nombre: usuarioData.nombre }); await cargarNotas(); showToast(nota.archivada ? 'Nota desarchivada' : 'Nota archivada', 'success'); }
            ...
            <div className="bg-white rounded-2xl shadow-sm border-2 p-6">
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div>
                  ...
                  <input type="text" value={formData.clienteNombre} onChange={(e) => setFormData({ ...formData, clienteNombre: e.target.value })} className="w-full px-4 py-2 rounded-lg border-2 border-gray-200 focus:border-purple-400 focus:outline-none" placeholder="Nombre del cliente" />
                </div>
                <div>
              ...
              <div className="mb-6">
                ...
                <input type="text" value={formData.evento} onChange={(e) => setFormData({ ...formData, evento: e.target.value })} className="w-full px-4 py-2 rounded-lg border-2 border-gray-200 focus:border-purple-400 focus:outline-none" placeholder="Cumpleaños, Boda, etc." />
              </div>
                    ...
                    <div key={trabajo.id} className="bg-gray-50 rounded-xl p-4 border-2 border-gray-200">
                      ...
                      <div className="grid grid-cols-2 gap-3 mb-3">
                        <div className="col-span-2">
                          <input type="text" value={trabajo.producto} onChange={(e) => actualizarTrabajo(trabajo.id, 'producto', e.target.value)} placeholder="Producto/Servicio" className="w-full px-3 py-2 rounded-lg border border-gray-300 focus:border-purple-400 focus:outline-none" />
                        </div>
